...
function App() {
  ...
  // ESTADO NUEVO: Controla si vemos la lista o el formulario
  ...
  const handleSubmitVal = async (e) => {
    ...
    // El .trim() elimina espacios en blanco al inicio y final para evitar que envíen solo espacios.
    if (
      !formData.Nombre_infiel.trim() || 
      !formData.Ciudad_origen ||          // Valida el Combobox Ciudad
      !formData.Sexo ||                   // Valida el Combobox Sexo
      !formData.Edad || 
      ...
      return; // Detiene la ejecución aquí, no envía nada al servidor
    }
  ...
  useEffect(() => {
    ...
    const pingServer = async () => {
      try {
        await fetch(URL_PRODUCCION);
        console.log(`💓 Ping enviado a Render: ${new Date().toLocaleTimeString()}`);
      } catch (error) {
        console.error("⚠️ Error en ping de mantenimiento", error);
      }
    };
    ...
    <div className="main-container">
      <style>{`
        ...
        .action-btn {
          ...
          border-radius: 12px; font-weight: 600; cursor: pointer; transition: 0.2s; margin-bottom: 20px;
          font-size: 1rem;
        }
        ...
        .card { background-color: white; border-radius: 16px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); border: 1px solid #f1f5f9; cursor: pointer; transition: transform 0.2s; display: flex; flex-direction: column; }
        .card:hover { transform: translateY(-5px); box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1); }
        .card-header { padding: 20px; display: flex; justify-content: space-between; background-color: #f8fafc; border-bottom: 1px solid #f1f5f9; }
        ...
        .view-btn { width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #e2e8f0; background: transparent; color: #0f172a; font-weight: 600; cursor: pointer; }
        ...
        <div className="search-section">
          {vista === 'lista' ? (
            <>
              ...
              <div style={{display: 'flex', alignItems: 'center', gap: '10px', fontSize: '0.85rem', color: '#94a3b8', marginTop: '20px'}}>
                <span style={{width: '8px', height: '8px', borderRadius: '50%', background: cargando ? '#fbbf24' : '#34d399'}}></span>
      ...
      {/* --- MODAL DETALLE PRO (DISEÑO MEJORADO) --- */}
      {personaSeleccionada && (
